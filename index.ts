...
import { readdirSync, writeFile, existsSync, mkdirSync, writeFileSync } from "fs";
...
if (!existsSync("./cookies.json")) {
  ...
  console.log(
    "To enable, create a cookies.json file in root with your session cookie formatted like this:"
  );
...
const inputFile = ["."].concat(year).concat(day).concat("input.txt").join("\\");

if (!existsSync(inputFile)) writeFileSync(inputFile, "");

readFile(inputFile, "utf8")
  .then((inputData) => {
    if (inputData.length > 0 && !reloadInput) {
      console.log(
        "Inputfile populated. To redownload, specify r as the third argument (after day)."
      );
    } else {
      readFile("./cookies.json", "utf8").then((cookieFile) => {
        ...
        fetch(`https://adventofcode.com/${year}/day/${day}/input`, {
          ...
          .then((text) => {
            ...
            while (text.charCodeAt(text.length - 1) === 10)
              text = text.slice(0, -1);
  ...
  .finally(() => {
    if (!init) {
      console.log(
        "Not reinitializing. To reinitialize, use i as the third argument (after day)."
      );
    } else {
    ...
    spawn("tsx.cmd", ["watch", `./${year}/${day}/index.ts`, year, day], {
      stdio: "inherit",
      shell: true,
      env: { YEAR: year, DAY: day },
    });
  });
